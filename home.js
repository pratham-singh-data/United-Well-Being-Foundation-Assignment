...
const changeCard = (move) => {
  ...
  reviewsCard.innerHTML = `
        <div class="review-card-image">
            <img src=${
              data[currentCard].image
                ? data[currentCard].image
                : getDefaultProfile()
            } alt=${data[currentCard].name}/>
            <p style="color: #6D2E46; margin-top: 10px;">${
              data[currentCard].name
            }</p>
        </div>

        <div class="review-card-content">
            <div id="stars-display"></div>
            <p class="review-card-content-text-stars">${
              data[currentCard].stars
            }/5</p>
